...
WITH activity
     AS (SELECT b.ownerid                      AccountOwnerID,
         ...
         FROM   (
                 ...
                 AND (PC.activitytypecode LIKE '%walk-in%' OR PC.activitytypecode LIKE '%face appointment%')
                 OR PC.activitytypecode LIKE '%phone%'
                 OR PC.activitytypecode LIKE '%email%'
                 OR PC.activitytypecode LIKE '%letter%'
                 ) AS b
                INNER JOIN kslcloud_mscrm.dbo.contact a WITH (nolock)
         ...
         FROM   (SELECT TOP 1000 activityid,
                                 ksl_resultoptions_displayname AS Rslt,
                                 pc.ownerid,
                                 pc.[from],
                                 PC.subject,
                                 PC.activitytypecode,
                                 PC.activitytypecode        AS
                                 ActivityTypeDetail,
                                 PC.regardingobjectid,
                                 PC.scheduledstart             AS CompletedDate,
                                 c.moveindate,
                 ...
                 FROM   KSLCLOUD_MSCRM_RESTORE_TEST.dbo.activities PC WITH (nolock)
                        JOIN [DataWarehouse].[dbo].[fact_lead] c
                 ...
                 WHERE  PC.statuscode_displayname = 'Completed'
                        ...
                        AND scheduledstart BETWEEN Datefromparts(2025, 1, 1)
                                                   ...
                                                   AND CASE
                                                       ...
                                                       END) e),
     actsum
     AS (SELECT accountownerid,
                Sum(CASE
                      WHEN ([activitytype] LIKE '%Face Appointment%' OR [activitytype] LIKE '%Walk-in%') THEN 1
                      ELSE 0
                    END)       AS appointment_BD,
                Sum(CASE
                      WHEN [activitytype] = 'RR Bus Development Drop In BD' THEN 1
                      ELSE 0
                    END) * 1.0 AS RR_appointment_BD,
                Sum(CASE
                      WHEN [activitytype] LIKE '%email%' THEN 1
                      ELSE 0
                    END)       AS email_BD,
                Sum(CASE
                      WHEN [activitytype] LIKE '%letter%' THEN 1
                      ELSE 0
                    END)       AS letter_BD,
                Sum(CASE
                      WHEN [activitytype] LIKE '%phone%' THEN 1
                      ELSE 0
                    END)       AS phonecall_BD
         FROM   activity
         GROUP  BY accountownerid),
     nrr
     AS (SELECT Sum(rentrev) RentRev,
                ksl_securityregionteamid
         FROM   (SELECT Isnull(Sum(ksl_act_commtransfee +
new_apartmentrate -
                                  Isnull(est.ksl_act_commtransfeespecial, 0)), 0) AS RentRev,
a.ksl_communityid
 FROM   (SELECT afh.ksl_begindate,
                afh.ksl_apartmentid,
                ...
                Max(afh.ksl_moveoutdestinationidname) AS
                ksl_MoveOutDestinationIdName,
         ...
         WHERE  ( afh.ksl_begintransactiontype IN (
                  864960001, 864960003, 864960007, 864960008 )
                  -- 01=Actual Move In, 03=Actual Transfer In, Scheduled Transfer, Scheduled Move In
                  AND a.[ksl_initialsourcecategory] =
                      '25AC1CB4-C27F-E311-986A-0050568B37AC'
                  AND afh.statecode = 0
                  AND ( afh.ksl_endtransactiontype IN (
                        864960004, 864960006, 864960002,
                        864960005 )
                         OR afh.ksl_endtransactiontype IS NULL ) )
                 -- Actual Transfer Out, Actual move out, Scheduled Transfer,Scheduled move out
                 OR ( afh.ksl_begintransactiontype = 864960000
                      AND afh.statecode = 0
                      AND afh.ksl_endtransactiontype IS NULL
                      AND Cast(afh.ksl_begindate AS DATE) >=
         ...
         GROUP  BY afh.ksl_begindate,
                   ...
                   --,afh.AccountId
                   ,
                   afh.ksl_communityidname) AS y
        FULL OUTER JOIN [quote] est
                     ON quoteid = ksl_estimateid
        LEFT JOIN account A WITH (nolock)
               ON a.accountid = est.customerid
 WHERE  COALESCE(CASE
                   WHEN y.ksl_begintransactiontype = 864960001 THEN
                   'Actual Move in'
                   WHEN y.ksl_begintransactiontype = 864960003 THEN
                   'Actual Transfer In'
                   WHEN y.ksl_begintransactiontype = 864960007 THEN
                   'Short Term Stay Begin'
                   WHEN y.ksl_begintransactiontype = 864960008 THEN
                   'Seasonal Stay Begin'
                   WHEN y.ksl_begintransactiontype = 864960000 THEN
                   'Scheduled Move in'
                 --ELSE 'Other'
                 END, est.ksl_estimatetype_displayname) IN (
                'Actual Move in', 'Moved In' )
        AND a.[ksl_initialsourcecategory] =
            '25AC1CB4-C27F-E311-986A-0050568B37AC'
        ...
        AND Isnull(ksl_begindate, est.ksl_schfinanmovein) BETWEEN
            Datefromparts(@Year, 1, 1)
                                                                  ...
                                                                  AND CASE
                                                                        WHEN
                @Year = Year(Getdate()) THEN Getdate()
...
INNER JOIN [KSLCLOUD_MSCRM].[dbo].ksl_community c WITH (nolock)
 ...
 GROUP  BY ksl_securityregionteamid),
     leads
     AS (SELECT Count(accountid) AS newLeadsavg,
                ksl_securityregionteamid
         ...
         WHERE  [ksl_initialsourcecategory] =
                '25AC1CB4-C27F-E311-986A-0050568B37AC'
                ...
                AND ksl_initialinquirydate BETWEEN Datefromparts(@Year, 1, 1)
                                                   -- Start from January 1st of the selected year
                                                   AND CASE
                    WHEN @Year = Year(Getdate()) THEN Getdate()
                                                         -- If it's the current year, go up to today
                                                         ELSE Datefromparts(
                                                              @Year, 12
                                                              ,
         ...
         GROUP  BY ksl_securityregionteamid),
     moveins
     AS (SELECT Count(est.quoteid) AS MoveInavg,
         ...
         FROM   (SELECT afh.ksl_begindate,
                        ...
                        afh.ksl_estimateid,
                        afh.ksl_begintransactiontype,
                        Max(afh.ksl_endtransactiontype)
                        ksl_EndTransactionType,
                        Max(afh.ksl_reasondetailidname)       AS
                        ksl_ReasonDetailIDName,
                        Max(afh.ksl_moveoutdestinationidname) AS
                        ksl_MoveOutDestinationIdName,
                        Max(afh.ownerid)                      AS afh_OwnerID
                        --,LEA.ksl_ReasonDetailIdName AS ReasonDetail
                        ,
                        Max(ksl_moveoutreasondetailidname)    AS
                        MoveOutReasonDetail
                 FROM   ksl_apartmentfinancialhistory afh WITH (nolock)
                        --history of what happened 
                        LEFT JOIN account A WITH (nolock)
                ...
                LEFT JOIN ksl_apartment tra
                       ON est.ksl_act_transferfromapartmentid =
                          tra.ksl_apartmentid
                INNER JOIN [KSLCLOUD_MSCRM].[dbo].ksl_community c
                        ON a.ksl_communityid = c.ksl_communityid
         WHERE  COALESCE(CASE
     ...
     newrs
     AS (SELECT Count(contactid) RSourceAvg,
                c.createdby      ownerid
     ...
     resref
     AS (SELECT Count(accountid) * 1.0 RRAvg,
                [ksl_associtateduser],
...
SELECT u.USR_First + ' ' +  u.USR_Last as fullname,
       CASE
         WHEN r.Name LIKE 'Business Development Director'
               ...
               OR r.Name LIKE 'Director of Strategic Partnership%' THEN
         'Business Development'
         WHEN r.Name LIKE 'Executive%'
               OR r.Name LIKE 'General Manager' THEN 'Executive Director'
         ELSE 'Sales'
       END         Title,
