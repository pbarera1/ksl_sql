...
SELECT
    c.Community                 AS ksl_name,
    (u.USR_First + ' ' + u.USR_Last) AS fullname,                             -- UPDATED (was systemuser.fullname)
    b.description,
    c.ksl_communityid,
...
INNER JOIN (
    SELECT
        CASE WHEN ShortName = 'KSL' THEN 'HO' ELSE ShortName END AS ShortName,
        ksl_communityid,
...
WHERE MONTH(CONVERT(date, b.dt)) = MONTH(@AsOfDate)
  ...
  AND budget = '0'
  AND (c.ksl_communityid = @comm OR @comm = '27C35920-B2DE-E211-9163-0050568B37AC')

UNION ALL

-- “Home Office” specialists block
SELECT
    'Home Office'                                        AS ksl_name,
    (u.USR_First + ' ' + u.USR_Last)                     AS fullname,          -- UPDATED
    u.USR_Email                                          AS description,       -- UPDATED (was internalemailaddress)
...
INNER JOIN KiscoCustom.dbo.Community kc
    ON kc.CommunityIDY = u.USR_CommunityIDY
INNER JOIN KSLCLOUD_MSCRM_RESTORE_TEST.dbo.ksl_community commCrm
    ON commCrm.ksl_communityid = kc.CRM_CommunityID
-- Optional: role/title filter moved to KSL_Roles if you used systemuser.title previously                           -- UPDATED
LEFT JOIN KiscoCustom.dbo.KSL_Roles r
    ON r.RoleID = u.RoleID
WHERE (commCrm.ksl_communityid = @comm OR @comm = '27C35920-B2DE-E211-9163-0050568B37AC')  -- UPDATED (was u.ksl_communityid)
  AND (
