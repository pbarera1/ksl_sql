...
--set @DimUserFullName = 'robin howland'
...
WHERE coalesce(CASE 
	...
	AND isnull(ksl_BeginDate, est.ksl_schfinanmovein) BETWEEN DATEADD(month, DATEDIFF(month, 0, dateadd(day, - 1, getdate())), 0)
		AND EOMONTH(dateadd(day, - 1, getdate()))
	AND a.ownerid IN (
		SELECT [systemuserid]
		FROM [KSLCLOUD_MSCRM].[dbo].[systemuser]
		WHERE --[yomifullname] like @DimUserFullName
			[yomifullname] = replace(substring(ltrim(rtrim(@DimUserFullName)), 25, 100), ']', '')
		)