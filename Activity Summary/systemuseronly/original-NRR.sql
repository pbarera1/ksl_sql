SELECT convert(VARCHAR(50), month(isnull(ksl_BeginDate, est.ksl_schfinanmovein))) + '/1/' + convert(VARCHAR(50), year(isnull(ksl_BeginDate, est.ksl_schfinanmovein))) AS dt,
	...
	(
		...
		INNER JOIN kslcloud_mscrm.dbo.systemuser a ON a.internalemailaddress = b.description
		WHERE firstname + ' ' + lastname IN (replace(substring(ltrim(rtrim(@DimUserFullName)), 25, 100), ']', ''))
