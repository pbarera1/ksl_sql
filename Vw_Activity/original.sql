USE [DataWarehouse] 
--GO
 /****** Object:  View [dbo].[Vw_Activities]    Script Date: 10/10/2025 12:58:30 PM ******/ 
--SET ANSI_NULLS ON
--GO
     ...
     (SELECT pc.activityid,
      ...
      WHERE PC.statecode_displayname = 'Completed'
        AND PC.ksl_appointmenttype IN (864960001,
                                       864960002)
        AND PC.ksl_resultoptions IN ('864960005',
                                     '864960004',
                                     '864960006')--Result: 864960005:Completed  864960004:Community Experience  864960006: Virtual Experience
) AS b)
...
,
       CASE
           WHEN (ActivityType = 'phonecall'
                 OR (ActivityTypeDetail = 864960000
                     AND ActivityType = 'appointment')/*Phone Appointment*/)
                AND Rslt ='COMP - Completed'
                AND ActivityTypeDetail <> 864960000 /*incoming call*/ THEN 1
           ELSE 0
       ...
       CASE
           WHEN (ActivityType = 'email'
                 AND ActivityTypeDetail IN(864960002))
                OR (ActivityType = 'letter'
                    AND ActivityTypeDetail IN(864960000)) THEN 1
           ELSE 0
       ...
       CASE
           WHEN ActivityType = 'Appointment'
                ...
                AND CAST(CompletedDate AS DATE) <> CAST(LastCEDate AS DATE) THEN 1
           ELSE 0
       END AS Appointment,
       CASE
           WHEN ActivityType = 'email BD'
                AND ActivityTypeDetail =864960002 THEN 1
           ELSE 0
       ...
       CASE
           WHEN ActivityType = 'Appointment BD'
                AND (Rslt ='COMP - Completed'
                     OR Rslt ='CEXP - Community Experience Given') THEN 1
           ELSE 0
       END AS Appointment_Biz_Dev,
       CASE
           WHEN ActivityType = 'Appointment'
                AND (Rslt ='CEXP - Community Experience Given'
                     OR (ActivityTypeDetail IN (864960001,
                                                864960002)
                         AND Rslt = 'COMP - Completed'))
                AND CAST(CompletedDate AS DATE) = CAST(LastCEDate AS DATE) THEN 1
           ELSE 0
       ...
       CASE
           WHEN ActivityType = 'Appointment'
                AND Rslt ='VEXP - Virtual Comm Exp Given' THEN 1
           ELSE 0
...
a.OwnerId AccountOwnerID,
                                                               END AS TextSent a.OwnerIdName AccountOwnerName,
 ...
 isnull(dateadd(HOUR,
                  (SELECT com.[ksl_utctimeadjust]
                   ...
                   WHERE ksl_communityid = a.ksl_communityid) , b.CompletedDate), b.CompletedDate) CompletedDate,
 Rslt,
 activityid,
 notes,
 ksl_textssent,
...
UNION ALL SELECT activityid,
      ...
      UNION ALL SELECT activityid,
                       ...
                       ksl_textsreceived
      FROM kslcloud_mscrm.dbo.Account L WITH (NOLOCK)
      INNER JOIN kslcloud_mscrm.dbo.ksl_sms PC WITH (NOLOCK) ON PC.RegardingObjectId = L.accountid --SELECT L.accountid, RIGHT(PC.Subject, LEN(PC.Subject) - 13) as [Subject], 'sms' as ActivityTypeCode, NULL as ActivityTypeDetail, PC.regardingobjectid, PC.actualend as CompletedDate, pc.description as notes
...
UNION ALL SELECT activityid,
      ...
      UNION ALL SELECT activityid,
                       ...
                       ksl_textssent,
                       ksl_textsreceived
      FROM kslcloud_mscrm.dbo.ksl_sms PC WITH (NOLOCK)--Union All
